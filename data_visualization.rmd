...
completeFun(dat,c("CES 3.0 Percentile","rebate_capita_average","rebate_capita_average_before","rebate_capita_average_after")) %>%filter(rebate_capita_average < 100)%>%ggplot()+
  geom_smooth(aes(x=`CES 3.0 Percentile`,y=`rebate_capita_average_before`,color="before policy",linetype="before policy"),alpha = 0.25)+
  geom_smooth(aes(x=`CES 3.0 Percentile`,y=`rebate_capita_average_after`,color="after policy",linetype="after policy"),alpha = 0.25)+
  ...
  ggtitle("Rebate per capita ordered by CES percentile")+
...
completeFun(dat,c("income_md","rebate_capita_average","rebate_capita_average_before","rebate_capita_average_after")) %>%filter(rebate_capita_average < 100)%>%ggplot()+
  geom_smooth(aes(x=`income_md`,y=`rebate_capita_average_before`,color=as.factor("before income cap"),linetype=as.factor("before income cap")),alpha = 0.25)+
  geom_smooth(aes(x=`income_md`,y=`rebate_capita_average_after`,color=as.factor("after income cap"),linetype=as.factor("after income cap")),alpha = 0.25)+
  xlab("Median income")+
  ylab("Rebate per capita")+
  labs(color =NULL,linetype= NULL)+
  ggtitle("Rebate per capita ordered by median income")+
  scale_color_aaas()+
  theme_bw()+
  theme(legend.position="bottom")+
  ggsave("Figures/income_capita.pdf",width = 8.7/2, height = 8.7/3, dpi = 300)
